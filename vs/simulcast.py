...
from vsdeband import F3kdb, Placebo, deband_detail_mask
...
placebo_args: dict[str, Any] = dict(iterations=2, thr=4, radius=16, grain=0)

if video_in_dh <= 810:  # type:ignore
    f3kdb_arg.update(radius=16, threshold=16)
    placebo_args.update(thr=3, radius=12)
...
grain = core.grain.Add(deband, 2, constant=True)

depth(grain, get_depth(video_in)).set_output()  # type:ignore